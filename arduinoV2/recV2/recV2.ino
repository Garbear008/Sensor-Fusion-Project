...
// sound transmission (don't work, use a newer version for sound)
const int dotDuration = 60; // time duration for each short beep (dot or "0" in binary) 200 150
const int dashDuration = dotDuration * 3; // time duration for each long beep (dash or "1" in binary)
const int charGap = dotDuration * 6; // gap in time for each character (or after each byte) 6
const int durationMs = dotDuration / 4; // time in milliseconds to complete the sampling of the sound 50
const int sampleIntervalMs = dotDuration / 200; // time in milliseconds it takes between each sample 

...
// for sending sound in microseconds
const long int dotDuration = 1000; // time duration for each short beep (dot or "0" in binary)
const long int dashDuration = dotDuration * 3; // time duration for each long beep (dash or "1" in binary)
const long int bitGap = dotDuration * 3; // time gap between each bit
const long int charGap = dotDuration * 6; // time gap between each character
...
void messageToTone(String msg) {
  ...
  for (int i = 0; i < msg.length(); i++) {
    ...
    delayMicroseconds(charGap); // time between each character
  }
}
...
void zeroAndOne(char c) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    delayMicroseconds(bitGap); // time between each bit
  }
...
void signal(int duration) {
  ...
  delayMicroseconds(duration); // play the tone for a amount of time (via delay)

...
// sound transmission in microsconds
const long int dotDuration = 1000; // time duration for each short beep (dot or "0" in binary) 
const long int charGap = dotDuration * 3; // gap in time for each character (or after each byte)
const long int bitGap = dotDuration * 2;
const long int durationMicros = 10; // time in microsconds to complete the sampling of the sound
const int sampleIntervalMicros = 1; // time in microsconds it takes between each sample 
...
void loop() {
  ...
  if (digitalRead(buttonPin)==HIGH) // LOW = button is pressed; HIGH = button is not pressed
  {
    // light mode
  ...
  else
  {
    ...
    if (pickUpSound(durationMicros, sampleIntervalMicros)) { // if sound is detected
      unsigned long start = micros(); // start time

      while (true) {
      if (!pickUpSound(durationMicros, sampleIntervalMicros)) { // while silence
        unsigned long silenceStart = micros();
        bool stableSilence = true;

        while (micros() - silenceStart < bitGap) { // if length of silence is less than gap between bits
          if (pickUpSound(durationMicros, sampleIntervalMicros)) {
            stableSilence = false; // then this silence is not a bit gap
            break;
          }
        }

        if (stableSilence) break; // means silence is a bit gap if break is not executed above
      }
    }
      ...
      while (!pickUpSound(durationMicros, sampleIntervalMicros)) { // detects for the absence of sound (gap)

...
void zeroAndOne(char c) {
  ...
  for (int i = 0; i < 8; i++) {
    if (bitRead(c, 7 - i)== 0) {
      signal(dotDuration, 1000); // calls to play a short beep if "0"
      //Serial.print(0); // for troubleshooting
    } else {
      signal(dashDuration, 2300); // calls to play a long beep if "1"

...
const long durationMicros = 50;       // Duration (in microseconds) for sampling sound once
...
const long charGap = fourDotDuration * 1.5;  // Gap between characters (or after each byte) in 4-bit mode
...
void printAll() {
  ...
  double correctLightPercentage = (double)realLightString.length() / (realLightString.length() + trashLightString.length());
  double correctSoundOnePercentage = (double)realSoundOneString.length() / (realSoundOneString.length() + trashSoundOneString.length());
  double correctSoundFourPercentage = (double)realSoundFourString.length() / (realSoundFourString.length() + trashSoundFourString.length());
